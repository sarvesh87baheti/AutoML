...
def _build_pipeline(scale: bool = True, **est_kwargs) -> Pipeline:
    # Scaling does not affect GaussianNB predictions mathematically, but kept
    # here for pipeline consistency so the model receives normalised inputs.
    if scale:
        return Pipeline([("scaler", StandardScaler()), ("est", GaussianNB(**est_kwargs))])
    return Pipeline([("est", GaussianNB(**est_kwargs))])

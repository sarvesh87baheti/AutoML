...
def _build_pipeline(scale: bool = True, **est_kwargs) -> Pipeline:
    if scale:
        return Pipeline([("scaler", StandardScaler()), ("est", MLPClassifier(**est_kwargs))])
    return Pipeline([("est", MLPClassifier(**est_kwargs))])
...
def train_model(
    ...
    **kwargs,
    ...
    # Sensible defaults — override via kwargs
    est_kwargs = {
    ...
    pipe = _build_pipeline(scale=scale, **est_kwargs)
    pipe.fit(X_train, y_train)

    metrics = {"train": evaluate_classification_model(pipe, X_train, y_train)}
    if X_val is not None and y_val is not None:
        metrics["val"] = evaluate_classification_model(pipe, X_val, y_val)

    metadata = {"name": MODEL_NAME, "hyperparams": est_kwargs, "train_samples": int(len(X_train))}

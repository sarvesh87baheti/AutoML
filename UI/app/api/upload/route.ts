...
// Error codes that are the caller's fault (→ 400) vs our fault (→ 500).
...
export async function POST(req: Request) {
  try {
    ...
    // ── Custom script fields (up to MAX_CUSTOM_SCRIPTS) ───────────────────
    const customScriptFiles: File[] = [];
    for (let i = 0; i < MAX_CUSTOM_SCRIPTS; i++) {
      const f = form.get(`custom_script_${i}`) as File | null;
      if (f && f.size > 0) customScriptFiles.push(f);
    }
    ...
    if (customScriptFiles.length > MAX_CUSTOM_SCRIPTS) {
      return NextResponse.json(
        {
          success: false,
          error: {
            code: "TOO_MANY_SCRIPTS",
            message: `At most ${MAX_CUSTOM_SCRIPTS} custom scripts are allowed per run.`,
          },
        },
        { status: 400 }
      );
    ...
    for (const sf of customScriptFiles) {
        ...
        return NextResponse.json(
          {
            ...
            error: {
              ...
              message: `Custom scripts must be .py files (got '${sf.name}').`,
            },
          },
          { status: 400 }
        );
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            message: "Target column required for supervised tasks",
          },
        },
        { status: 400 }
      );
    ...
    if (problem_type === "kmeans_clustering" && !k_value) {
      return NextResponse.json(
        {
          success: false,
          error: { code: "K_REQUIRED", message: "k is required for kmeans clustering" },
        },
        { status: 400 }
      );
    }
    ...
    if (
      ...
      image_mode &&
      !["light", "standard"].includes(image_mode)
    ) {
      return NextResponse.json(
        {
          success: false,
    ...
    // ── Custom scripts blocked for image classification ────────────────────
    if (problem_type === "image_classification" && customScriptFiles.length > 0) {
      return NextResponse.json(
        {
          success: false,
          error: {
            code: "INVALID_PROBLEM_TYPE",
            message: "Custom scripts are not supported for image classification.",
          },
        },
        { status: 400 }
      );
    ...
    // ── Save custom scripts ────────────────────────────────────────────────
    // Each run gets its own subfolder keyed by timestamp so concurrent runs
    // don't overwrite each other's scripts.
    const savedScriptPaths: string[] = [];

    if (customScriptFiles.length > 0) {
      const sessionId = Date.now().toString();
      const scriptDir = path.join(workspaceRoot, "uploaded_files", "custom_scripts", sessionId);
      await fs.mkdir(scriptDir, { recursive: true });

      for (const sf of customScriptFiles) {
        const scriptPath = path.join(scriptDir, sf.name);
        const scriptBuffer = await sf.arrayBuffer();
        await fs.writeFile(scriptPath, Buffer.from(scriptBuffer));
        savedScriptPaths.push(scriptPath);
      }
    }

    // ── Build runner.py args ───────────────────────────────────────────────
    ...
    if (target_col)  args.push("--target", target_col);
    if (k_value)     args.push("--k", k_value);
    if (image_mode)  args.push("--image-mode", image_mode);

    // Append custom script paths as --custom-scripts p0 p1 p2
    if (savedScriptPaths.length > 0) {
      args.push("--custom-scripts", ...savedScriptPaths);
    }
    ...
    if (result && result.success === false) {
      const status = USER_ERROR_CODES.has(result.error?.code) ? 400 : 500;
...
// ---------------------------------------------------------------------------
// Error parsing (extended with custom-script messages)
// ---------------------------------------------------------------------------
...
function parsePythonError(stderr: string) {
  ...
  if (
    ...
    message =
      "Python was not found on the server. Install Python and make sure `python3` or `python` is available on PATH.";
  } else if (text.includes("terminated by signal SIGKILL")) {
    code    = "PROCESS_KILLED";
    message =
      "Training was stopped by the operating system, likely due to high memory usage. Try a smaller image dataset or the lighter default image models.";
  } else if (text.includes("Target column") && text.includes("not found")) {
    code = "TARGET_COLUMN_NOT_FOUND";
    const match = text.match(/Target column '([^']+)'/);
    message = match
      ? `Target column "${match[1]}" was not found in the uploaded dataset.`
      : "Specified target column was not found in the uploaded dataset.";
    ...
    text.includes("ZIP contains no CSV/XLSX") ||
    text.includes("ZIP contains no image")
  ) {
    code    = "EMPTY_ZIP";
    message = "ZIP file does not contain valid data or images.";
  } else if (
    text.includes("Invalid image ZIP") ||
    (text.includes("image classification") && text.includes("invalid"))
  ) {
    code    = "INVALID_IMAGE_ZIP";
    message =
      "ZIP structure invalid. Expected: folders for each class with images inside.";
...
// ---------------------------------------------------------------------------
// Python subprocess helpers (unchanged from original)
// ---------------------------------------------------------------------------
  ...
  return new Promise((resolve) => {
    ...
    proc.on("close", (code, signal) => {
      ...
      try {
        ...
        resolve({
          success: true,
          data: { message: "AutoML process completed", raw: stdout },
        });
      }
    });
  });
